...
const MemberTable = () => {
  ...
  const { data, isLoading } = api.user.getAllStaff.useQuery({
  ...
  const column: ColumnDef<User>[] = [
    ...
    { accessorKey: "email", header: "Email" },
    {
      ...
      header: "Role",
      cell(props) {
        return <div className="capitalize">{props.row.original.role}</div>;
      },
    },
    {
      id: "actions",
  ...
  return (
    <>
      <div className="flex w-full justify-between">
        ...
        <HiButton onClick={handleOpenModal}>Add Member</HiButton>
      </div>
      ...
      <HiModal
        title={`${editableData ? "Edit" : "Create New"} Member`}
        open={openModal}
        onClose={closeModal}
      >
        <EditForm
